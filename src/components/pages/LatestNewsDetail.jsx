...
const LatestNewsDetail = () => {
    ...
    const [{ title, img, tag, time, comment }] = filtered
        ...
        <section className="w-full h-auto flex flex-col gap-8 lg:px-20 md:px-8 px-6 md:mt-16 mt-12">
            <main className="w-full grid lg:grid-cols-3 md:grid-cols-2 gap-6">
                <div className="flex flex-col col-span-2 h-auto gap-2 ">
                    <h1 className="font-bold md:text-4xl text-2xl capitalize text-gray-100">{title}</h1>
                    <div className="w-full h-[250px]">
                        <img src={img} className="w-full h-full object-cover object-top" alt="image" />
                    </div>
                ...
                <div className="flex flex-col gap-6">
                    <h1 className="font-bold md:text-3xl text-xl capitalize text-gray-100">Related Posts</h1>
                    {
                        related.map(blog => (
                            <div key={blog.id} className="flex flex-col h-auto gap-2 bg-gray-900">
                                <div className="w-full h-[250px]">
                                    <img src={blog.img} className="w-full h-full object-cover object-top" alt="image" />
                                </div>
                                <div className="flex flex-col gap-4 px-5 pt-4 pb-6">
                                    ...
                                    <Link to={`/latestnews/${blog.id}`} className="text-gray-100">{blog.title}</Link>
